...
test20()
{
  ...
  for(i = 0; i < 20; i++){
    status[i] = 0;
    int cc = aio_write(fd, &i, sizeof(int), i*BSIZE, status+i);
    if(cc < 0){
      printf("aiotest: async write error at block %d\n", i);
      exit(-1);
    }
    if (status[i] != 0)
      n++;
  ...
  if (n == 20) {
    printf("aiotest: aio_write is not asynchronous\n");
    exit(-1);
  }
...
test1000()
{
  ...
  for (i = 0; i < 5; i++) {
    if (fork() == 0) {
      ...
      for (j = 0; j < 200; j++){
        status[j] = 0;
        int cc = aio_write(fds[i], &j, sizeof(int), j*BSIZE, status+j);
        if(cc < 0){
          printf("aiotest: async write error at block %d\n", j);
          exit(-1);
        }
